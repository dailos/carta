...
<div class="row">
	<div class="col-lg-8">
		...
		<h5>{{ sprintf("%05d", $ficha->cod_ficha) }} - {{ $ficha->denominacion }}@isset($ficha->actividad){{ ', ' . $ficha->actividad->nombre }}@endisset{{ isset($ficha->grupo) ? (', ' . $ficha->grupo->nombre) : '' }}</h5>
	</div>
	<div class="col-lg-4">
		<p>Si desea guardar la ficha en formato pdf pulse aquí</p>
		<a href="{{ route('fichas.download', $ficha->cod_ficha) }}" class="btn btn-block">Descargar</a>
	</div>
</div>
